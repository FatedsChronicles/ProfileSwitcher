...
ProfileDockWidget::ProfileDockWidget(QWidget *parent) : QWidget(parent)
{
	...
	// Quick Settings (Stream, Video, Output) — minimal, focused
	{
		...
		// Video (Base/Output res + FPS num/den)
		...
		// Output (mode + encoder hint for Simple mode)
...
void ProfileDockWidget::loadCurrentSettingsIntoForm()
{
	...
	// Stream (not all values live in profile config; service saved separately)
	// We don't read current server/key back for security; user can re-enter.
}

bool ProfileDockWidget::applySettingsToProfile(const QString &profileName)
{
	// switch context so writes target the selected profile
	...
	// --- Output (Mode + Simple encoder hint)
	...
	// --- Stream (optional RTMP server/key)
...
void ProfileDockWidget::onApplyQuickSettings()
{
	...
	// warn if outputs active (video reset will fail)
	if (outputsActiveWarn())
		return;

	if (!applySettingsToProfile(name)) return;

	// Reset video to apply res/FPS changes (requires outputs inactive)
